...
class Epics::ParseEbics < Faraday::Middleware
  ...
  def call(env)
    @app.call(env).on_complete do |response|
      ...
      begin
        File.write(Rails.root.join('transactions', "#{response[:body].transaction_id&.present? ? response[:body].transaction_id : DateTime.now.strftime('%Y%m%d%H%M%S')}.response.xml"), raw_body)
